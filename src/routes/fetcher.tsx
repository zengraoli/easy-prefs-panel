...
import { generateFetcherCode, type FetcherConfig, type SelectorRule } from "@/lib/codegen";
...
const fetcherTypes = [
  {
    ...
    icon: Zap,
    title: "快速抓取",
    desc: "适合简单网页，速度最快，资源消耗最小",
    tag: "推荐入门",
  },
  {
    type: "StealthyFetcher" as const,
    icon: ShieldCheck,
    title: "隐秘抓取",
    desc: "绕过 Cloudflare 等反爬机制，模拟真实浏览器指纹",
    tag: "反反爬",
  },
  {
    type: "PlayWrightFetcher" as const,
    icon: Clapperboard,
    title: "动态页面",
    desc: "处理需要 JavaScript 渲染的页面，支持页面交互",
    tag: "JS 渲染",
  },
];
...
function FetcherPage() {
  ...
  const addSelector = () => {
    update({
      selectors: [...config.selectors, { name: "", type: "css", value: "", attribute: "" }],
    });
  };
                    ...
                    <CardContent className="flex items-start gap-4 p-5">
                      ...
                      <div
                        className={`mt-1 h-4 w-4 rounded-full border-2 ${
                          config.type === ft.type ? "border-primary bg-primary" : "border-muted-foreground/30"
                        }`}
                      />
                    </CardContent>
                  </Card>
              ...
              <div className="space-y-4">
                <div>
                  <Label className="mb-1.5 flex items-center gap-1.5">
                    目标网址
                    <InfoTooltip text="输入你想抓取的网页完整地址，以 http:// 或 https:// 开头" />
                  </Label>
                  <Input
                    value={config.url}
                ...
                <div>
                  <Label className="mb-1.5 flex items-center gap-1.5">
                    超时时间（秒）
                    <InfoTooltip text="等待网页响应的最长时间，超过则放弃" />
                  </Label>
                  <Input
                    type="number"
                    min={1}
                  ...
                  <div className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <Label className="flex items-center gap-1.5">
                        无头模式
                        <InfoTooltip text="开启后浏览器在后台运行，不显示窗口。关闭可用于调试" />
                      </Label>
                ...
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <div>
                    <Label className="flex items-center gap-1.5">
                      使用代理
                      <InfoTooltip text="通过代理 IP 访问目标网站，避免被封禁" />
                    </Label>
                  ...
                  <CardContent className="space-y-3 p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        <Label className="mb-1 flex items-center gap-1 text-xs">
                          选择方式
                          <InfoTooltip text="CSS: 使用 CSS 选择器定位元素; XPath: 使用路径表达式; 文本搜索: 按文字内容查找" />
                        </Label>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={sel.type === "css" ? "h1.title" : sel.type === "xpath" ? "//h1" : "搜索文本"}
                    ...
                    {sel.type === "css" && (
                      <div>
                        <Label className="mb-1 flex items-center gap-1 text-xs">
                          提取属性（可选）
                          <InfoTooltip text="留空则提取文本内容，填写如 href 则提取链接地址" />
                        </Label>
                        <Input
                          value={sel.attribute || ""}
                          onChange={(e) => updateSelector(i, { attribute: e.target.value })}
                          placeholder="href / src / class（留空提取文本）"
                          className="h-8 text-sm"
                        />
                      </div>
                    )}
            ...
            <div className="space-y-5">
              ...
              {config.type !== "Fetcher" && (
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <div>
                    <Label>禁用额外资源</Label>
                    <p className="text-xs text-muted-foreground">不加载 CSS、字体等，提高速度</p>
                  </div>
                ...
                <>
                  <div className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <Label className="flex items-center gap-1.5">
                        屏蔽图片
                        <InfoTooltip text="不加载页面中的图片，加快抓取速度" />
                      </Label>
                      <p className="text-xs text-muted-foreground">节省带宽和流量</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <Label className="flex items-center gap-1.5">
                        模拟人类行为
                        <InfoTooltip text="模拟真实用户的鼠标移动和页面滚动，更难被检测" />
                      </Label>
                      <p className="text-xs text-muted-foreground">模拟鼠标和滚动行为</p>
                    </div>
                ...
                <>
                  <div className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <Label className="flex items-center gap-1.5">
                        禁用 WebGL
                        <InfoTooltip text="关闭 WebGL 可以减少浏览器指纹暴露" />
                      </Label>
                      <p className="text-xs text-muted-foreground">减少指纹暴露</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <Label className="flex items-center gap-1.5">
                        隐藏 Canvas
                        <InfoTooltip text="防止网站通过 Canvas 指纹识别你的浏览器" />
                      </Label>
                      <p className="text-xs text-muted-foreground">防止指纹识别</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <Label className="flex items-center gap-1.5">
                        等待网络空闲
                        <InfoTooltip text="等到页面所有网络请求完成后再提取数据，适合动态加载的页面" />
                      </Label>
                      <p className="text-xs text-muted-foreground">等待所有请求完成再提取</p>
                    </div>
              ...
              {config.type === "Fetcher" && (
                <Card className="border-dashed">
                  <CardContent className="py-8 text-center text-sm text-muted-foreground">
                    快速抓取模式暂无额外高级选项。如需更多控制，请切换到其他模式。
                  </CardContent>
                </Card>
              )}
            </div>
